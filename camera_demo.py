...
class ThreadingUpdatefacedb(object):
    """ Threading example class
    The run() method will be started and it will run in the background
    until the application exits.
    """
    def __init__(self, faces_dir, mtcnn_detector, interval=1):
        """ Constructor
        :type interval: int
        ...
        self.face_db = []
        thread = threading.Thread(target=self.run, args=(faces_dir, mtcnn_detector))
        thread.daemon = True                            # Daemonize thread
        thread.start()                                  # Start the execution

    def run(self, faces_dir, mtcnn_detector):
        """ Method that runs forever """
        while True:
            threadLock.acquire()
            self.face_db = load_faces(faces_dir,mtcnn_detector)
            threadLock.release()
...
def main():
    ...
    #cap = cv2.VideoCapture("http://operator:Abc12345@27.72.105.10:8933")
    ...
    last_time_save = datetime.datetime.now()
    time.sleep(10)
    first_reload = True
    ID = 0
    with tf.Graph().as_default():
                    ...
                    if faces.shape[0] is not 0:
                        ...
                        for i, embedding in enumerate(emb_arrays):
                            ...
                            if dict[0][1] > VERIFICATION_THRESHOLD:
                                ...
                                if (now-last_time_save).total_seconds() > 5:
                                    strname = str(ID)
                                    data_dir_name = "./face_db/" + strname
                                    if not os.path.exists(data_dir_name):   
                                        os.makedirs(data_dir_name)
                                    save_frame = frame.copy()
                                    for j,face in enumerate(faces):
                                        if j!=i:
                                            x1, y1, x2, y2 = faces[j][0], faces[j][1], faces[j][2], faces[j][3]
                                            x1 = max(int(x1), 0)
                                            y1 = max(int(y1), 0)
                                            x2 = min(int(x2), frame.shape[1])
                                            y2 = min(int(y2), frame.shape[0])
                                            cv2.rectangle(save_frame, (x1,y1) , (x2,y2) , (255, 0, 0) , -1 )
                                    cv2.imwrite(data_dir_name + "/" + strname +".jpg", save_frame)
                                    save = True
                                    ID =ID+1
                                sim = 0
                            names.append(name)
                ...
                if first_reload:
                    ...
                    if (now-last_time_save).total_seconds() >5:
                        try:
                            ...
                            print('error thread')
                    last_time_reload = now
                    first_reload =False
                else:
                    now = datetime.datetime.now()
                    if (now-last_time_save).total_seconds() >5 or (now-last_time_reload).total_seconds()>5:
                        try:
                            faces_db = load_face_db.face_db
                        except:
                            print('error thread')
